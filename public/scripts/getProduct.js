...
async function setProductCheckout() {
    ...
    if (window.location.pathname.endsWith('buy-listing')) {
        ...
        document.getElementById('product-info').innerHTML = `Fill out the form below to purchase ${product.units}x ${product.name} for ${product.price / 100} DKK.`;
    } else if (window.location.pathname.endsWith('success')) {
        document.getElementById('success-info').innerHTML = `Thank you for your purchase! <br> You have successfully purchased ${amount}x ${product.name} for ${product.price / 100} DKK${amount > 1 ? " each" : "."}`;
    }
}
